...
if premium:
    ...
    column_rename = {
        'date': 'Date', 'time': 'Time', 'heaviness': 'Heaviness (0-10)', 'pain': 'Pain (0-10)',
        'limb_appearance': 'Limb Appearance', 'measurement_taken': 'Measurement Taken',
        'affected_areas': 'Affected Areas', 'compression_type': 'Compression Type',
        'compression_hours': 'Compression Hours', 'self_care': 'Self Care',
        'dietary_triggers': 'Dietary Triggers', 'environmental_triggers': 'Environmental Triggers',
        'health_triggers': 'Health Triggers', 'stress': 'Stress (0-10)', 'sleep_quality': 'Sleep Quality',
        'energy': 'Energy (0-10)', 'mobility': 'Mobility (0-10)', 'self_compassion': 'Self-Compassion (0-10)',
        'biggest_challenge': 'Biggest Challenge', 'small_win': 'Small Win',
        'temperature': 'Temperature (°C)', 'humidity': 'Humidity (%)', 'tags': 'Tags'
    }
    df_export = df_export.rename(columns={k: v for k, v in column_rename.items() if k in df_export.columns})
    ...
    with pd.ExcelWriter(output, engine='openpyxl') as writer:
        ...
        thin_border = Border(
            left=Side(style='thin', color='DDE9E2'), right=Side(style='thin', color='DDE9E2'),
            top=Side(style='thin', color='DDE9E2'), bottom=Side(style='thin', color='DDE9E2')
        )
        
        for col_idx in range(1, len(df_export.columns) + 1):
            cell = worksheet.cell(row=1, column=col_idx)
            cell.font, cell.fill, cell.alignment, cell.border = header_font, header_fill, header_alignment, thin_border
        
        for row_idx in range(2, len(df_export) + 2):
            for col_idx in range(1, len(df_export.columns) + 1):
                cell = worksheet.cell(row=row_idx, column=col_idx)
                cell.alignment, cell.border = cell_alignment, thin_border
    ...
    <div style="background: #EAF3EE; padding: 1.5rem; border-radius: 16px; margin: 1rem 0; text-align: center;">
        <h3 style="color: #1A3B2E; margin-bottom: 0.5rem; font-family: 'Nunito', sans-serif;">$9.99 USD — One‑time payment</h3>
        <p>Lifetime access to formatted Excel exports, trend visualizations, and all future updates.</p>
    </div>
    """, unsafe_allow_html=True)
    if st.button("🌿 Go to Settings to Unlock", width='stretch'):
