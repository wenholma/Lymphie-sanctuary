...
st.warning("""
🔒 **Your data stays on this device.** This app works like a digital notebook — everything you enter is saved only in your browser. **The Lymphie Sanctuary does not store, transmit, or have access to your health data.** ⚠️ This tool is for personal tracking only. It is not medical advice. Always consult your healthcare provider.
""")
...
with st.form("daily_log_form"):
    ...
    with col1: entry_date = st.date_input("Date", value=datetime.now().date(), key="form_date")
    with col2: entry_time = st.time_input("Time", value=datetime.now().time().replace(second=0, microsecond=0), key="form_time")
    st.divider()
    ...
    appearance = st.selectbox("How did the limb look today?", ["Baseline / Normal", "Slight puffiness (pitting)", "Noticeable swelling (firm)", "Marked swelling / skin stretched"], key="form_appearance")
    info_icon("Baseline = your usual good day; marked = tight, shiny skin")
    st.divider()
    st.subheader("📏 Measurements")
    measurement = st.radio("Did you take a limb circumference measurement today?", ["Yes — full measurement", "Yes — partial measurement", "No", "Not applicable"], horizontal=True, key="form_measurement")
    st.divider()
    st.subheader("📍 Affected Areas")
    affected_areas = st.multiselect("Select all that apply", ["Left arm", "Right arm", "Left leg", "Right leg", "Trunk", "Head/neck", "Genital area", "Other"], key="form_areas")
    info_icon("Select all areas where you noticed swelling or symptoms today.")
    st.divider()
    st.subheader("🧦 Compression & Self-Care")
    col1, col2 = st.columns(2)
    with col1:
        compression = st.selectbox("Compression worn today", ["Not applicable", "None", "Light support (OTC)", "Circular knit (standard)", "Flat knit (custom)", "Bandages / wraps", "Night garment", "Kinesio taping"], key="form_compression")
        compression_hours = st.slider("Hours of compression worn today", 0, 24, 8, key="form_comp_hours")
    with col2:
        self_care = st.selectbox("Self-MLD / Dry brushing performed?", ["No", "Yes — Self MLD", "Yes — Therapist MLD", "Yes — Dry brushing", "Yes — Both", "Not applicable"], key="form_selfcare")
    st.divider()
    st.subheader("🍽️ Lifestyle & Triggers")
    diet_triggers = st.multiselect("Dietary triggers", ["High-salt meal", "High sugar", "Alcohol", "Caffeine", "Processed foods", "Dehydration", "Food intolerance", "Not applicable"], key="form_diet")
    env_triggers = st.multiselect("Environmental triggers", ["Heat exposure", "Cold exposure", "Long travel", "Long standing", "Long sitting", "Vigorous exercise", "Tight clothing", "Not applicable"], key="form_env")
    health_triggers = st.multiselect("Health triggers", ["Infection", "Menstrual cycle", "New medication", "Fatigue", "Poor sleep", "Stress spike", "Recent illness", "Not applicable"], key="form_health")
    st.divider()
    ...
    with col2:
        sleep_quality = st.selectbox("Sleep quality", ["Poor (<5h)", "Fair (5–7h)", "Good (7–8h)", "Very good (8+h)"], key="form_sleep")
    ...
    with st.expander("🌤️ Environment (optional)", expanded=False):
        col1, col2 = st.columns(2)
        with col1: temp = st.number_input("Temperature (°C)", value=None, step=0.1, key="form_temp")
        with col2: humidity = st.number_input("Humidity (%)", value=None, min_value=0, max_value=100, key="form_humidity")
...
if submitted:
    new_entry = {
        ...
        "Compression Type": compression, "Compression Hours": compression_hours, "Self Care": self_care,
        ...
        "Temperature": temp if temp is not None else "", "Humidity": humidity if humidity is not None else "", "Tags": tags
    }
    try:
        save_log(new_entry)
        logs = load_all_logs()
        ...
        st.caption("Your data is stored in this browser. Export regularly from the Export page to keep a backup.")
